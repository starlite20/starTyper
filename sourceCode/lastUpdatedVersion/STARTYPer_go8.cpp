...
void main()
{
	...
	cout<<endl
        <<endl<<"* * * * * * * * * * * * * * * * * * * *"
        <<endl<<" *         Welcome to *typer!          *";
    cout<<endl<<"  * * * * * * * * * * * * * * * * * * * *";

	cout<<endl
        <<endl<<"- - - - Are you a New Player? - - - -"<<endl
        <<endl<<"- - - - - 1. Yes - - - 2. No  - - - -"<<endl
         <<endl<<"->>";
	cin>>opt;
    cout<<endl;
  ...
  levelchoice:
	...
	if((levelnum>0)&&(levelnum<7))
	{
		...
		if(errors!=0)
		{
				if(errors>100)
					 cout<<endl<<"**************************************************"
                         <<endl<<"** You made "<<errors<<" Mistakes while typing! xD";
				else
					 cout<<endl<<"**************************************************"
                         <<endl<<"** You made "<<errors<<" Mistakes while typing!";
		  }
		scorer();
          
        cout<<endl<<"********************************"
		    <<endl<<"** The Time you took to type is "<<timer<<" seconds!!"
            <<endl<<"********************************"
...
void cheatzone()
{
  reccode:
	...
	if((strcmpi(ccode,"ST1698"))==0)
	{
		...
		while(fin.read((char*)&player,sizeof(player)))
		{
			if(strcmp(usernames,player.getusername())!=0)
			{
				cout<<endl
                    <<endl<<" * * * * * * * * * * * * * * * * * "
                    <<endl<<"We are modifying "<<player.getname()<<"'s Score";
				player.editscore();
				fout.write((char*)&player,sizeof(player));
			}
	...
	{
		cout<<endl<<" - - - - - - - - - - - - - - - - - - - - - - - - - - - "
            <<endl<<"  Invalid Cheat Code! Please Enter a Valid Cheat Code!"
            <<endl<<" - - - - - - - - - - - - - - - - - - - - - - - - - - - ";
		goto reccode;
	}
}
...
void backtomenu()
{
    ...
    cout<<endl<<endl
        <<endl<<" * * * * * * * * * * * * * * * * * * * * * * * * * *"
        <<endl<<"     Enter any number to get back to menu ->> ";
    cin>>bmenu;
}